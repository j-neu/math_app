...
**Status (2026-08-29):** All 92 items drafted (60 core + 32 deep-dive) by parallel subagents and verified: `check_provenance.py` → OK/0; full-bank `check_item_independence.py` → 1 flag (A3.3-02, adjudicated via sidecar, strict OK/0); all 13 template fields present in every item file; core counts match the blueprint allocation (sum 60). **R2.9 review completed 2026-08-30:** all 127 provenance rows carry `reviewed_by=Jakob`. The review struck A1.5-01 as redundant, taking the core tier from 60 to **59** — at the time both the runtime CSV and the live `cleanroom-v1` row still said 60. **Runtime side resolved 2026-09-04** (CSV regenerated to 59, independence sidecar re-keyed `20`→`19`, gates re-run green); only the live `cleanroom-v1` row remains at 60, with migrations `20260904000000_cleanroom_v1_core_59.sql` and `20260904000001_cleanroom_v1_sync_prompts.sql` written and pending apply. Delta tracked in `GAUNTLET_PROGRESS.md`.
...
- [x] **R2.9 — Jakob reviews the full item bank** ✅ 2026-08-30
  ...
  **Status:** Done 2026-08-30 (checkbox was stale; verified 2026-09-04). `provenance.csv` holds 127 rows — 59 core items, 32 deep-dive items, 36 skills — and **all 127 carry `reviewed_by=Jakob`, `reviewed_on=2026-08-30`**; zero pending. The review changed the bank: A1.5-01 was struck as redundant against A1.1-02, taking the core tier from 60 to 59 (recorded in `02-blueprint.md` and `decisions/0003`). **Open delta:** the runtime side is resolved (CSV regenerated to 59 on 2026-09-04, sidecar re-keyed `20`→`19`); the live `cleanroom-v1` row still carries 60 — migrations `20260904000000_cleanroom_v1_core_59.sql` + `20260904000001_cleanroom_v1_sync_prompts.sql` are written and pending apply. See the 59-vs-60 open item in `GAUNTLET_PROGRESS.md`.
