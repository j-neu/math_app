# [ARCHIVED] tasks.md — Clean-Room Rewrite: Immediate Task List

**Archived 2026-09-19.** Moved out of repo root so `tasks.md` could become a live planning
doc again. Fully historical for content purposes (see the 2026-09-12 and 2026-09-17 banners
below) — its only real surviving work items (Phase R9 legal review, Impressum/Datenschutz
placeholders) were carried forward into the new `tasks.md`. Read this file only for the
record of what the R0–R9 clean-room effort actually did.
...
**⛔⛔ Nachtrag 2026-09-12 — der gesamte Clean-Room-Ansatz ist von Jakob verworfen, nicht nur der v1-Output.** Sowohl dieser erste Durchlauf (R0–R7 unten) als auch der v2-Neuaufbau (Deckungsmatrix, Konstruktkarte, Itemregeln unter `docs/clean-room/v2/`) sind **eingestellt**, nicht nur inhaltlich korrigiert. Entscheidung: zurück zur ursprünglichen, iMINT/PIKAS-basierten Diagnostik und Skill-Taxonomie als Grundlage — `_sources_private/MathApp_Diagnostic_with_skills.csv` (92 Fragen) und `_sources_private/skills_taxonomy_legacy.csv` (88 Skills) — mit neu formuliertem Itemwortlaut je Skill (andere Zahlen, ein einfacher Satz, kein verratenes Verfahren; siehe die bereits erarbeitete Wortlaut-Regel in `docs/clean-room/v2/wortlaut-review-core.md`, die inhaltlich weiter gilt, nur jetzt auf die alte statt die neu abgeleitete Skill-Liste angewendet). **Rechtliche Erwägungen werden auf ausdrückliche Anweisung Jakobs vollständig ausgeklammert** — `rewrite.md` ist ab sofort ein historisches Dokument, keine aktive Vorgabe mehr. Der neue Plan läuft über `STATUS.md` „Active #1"; diese gesamte Datei (R0–R9) bleibt nur als Protokoll stehen, was in den zwei vorherigen Anläufen getan wurde.

**✅ Nachtrag 2026-09-17 — der oben angekündigte neue Plan wurde tatsächlich gebaut, nicht nur beschlossen.** Die iMINT/PIKAS-Rückkehr ist seit 2026-09-13 live (`content(v4)`-Commits: 108 Items in `math_app/Research/diagnostic_v4_master.csv`, 93-Skill-Taxonomie in `skills_taxonomy.csv`, `cleanroom-v1`/v2 aus der Laufzeit entfernt). Die dazugehörige Übungs-Pipeline (jede der 93 diagnostizierten Fähigkeiten braucht eigene Übungsstufen) ist seit 2026-09-17 in Arbeit über `docs/superpowers/plans/2026-09-15-exercise-plan-foundation.md`, gemerged nach `main` (2/93 Skills spielbar, Backlog für die restlichen 91 in `docs/clean-room/v4/skills/BUILD_ORDER.md`). **Diese gesamte Datei (R0–R9) ist damit endgültig nur noch Protokoll — für Inhalte gilt sie in keiner Weise mehr.** Einzige verbleibende reale Restarbeit aus dieser Liste: die Rechtsaufgaben in Phase R9 (Fachanwalt-Prüfung, externe Fachprüfung, kommerzieller Schalter) und die Impressum/Datenschutz-Platzhalter — beide unten weiterhin als offene Checkboxen geführt, auch in `STATUS.md`s Backlog-Liste referenziert. Aktueller Stand und nächste Schritte: `STATUS.md`.

**⛔ Nachtrag 2026-09-07 (weiterhin gültig als Befund, überholt als aktiver Plan) — der inhaltliche Output dieser Liste ist verworfen.** R1–R6 haben ein Diagnostikum produziert, das fachlich nicht brauchbar ist (fehlende Konstrukte, u. a. Verdoppeln/Halbieren im ZR20/ZR100; zu komplexe, verfahrensvorschreibende Items). Die abgehakten Kästchen unten beschreiben weiterhin korrekt, *was getan wurde* — sie bedeuten nicht mehr, dass das Ergebnis trägt. Bewertung und Befunde: `docs/clean-room/00-v1-assessment.md`. Die Neuentwicklung (v2) bekam eine eigene Spec und einen eigenen Plan; auch sie ist jetzt eingestellt (siehe Banner oben).
...
**Status (2026-08-29):** All 92 items drafted (60 core + 32 deep-dive) by parallel subagents and verified: `check_provenance.py` → OK/0; full-bank `check_item_independence.py` → 1 flag (A3.3-02, adjudicated via sidecar, strict OK/0); all 13 template fields present in every item file; core counts match the blueprint allocation (sum 60). **R2.9 review completed 2026-08-30:** all 127 provenance rows carry `reviewed_by=Jakob`. The review struck A1.5-01 as redundant, taking the core tier from 60 to **59** — at the time both the runtime CSV and the live `cleanroom-v1` row still said 60. **Delta fully resolved 2026-09-05:** the runtime side was fixed 2026-09-04 (CSV regenerated to 59, independence sidecar re-keyed `20`→`19`, gates re-run green), and the live migrations `20260904000000_cleanroom_v1_core_59.sql` and `20260904000001_cleanroom_v1_sync_prompts.sql` are **confirmed applied** to the live project (`supabase migration list` 2026-09-05) — live `cleanroom-v1` reports `question_count=59` with 59 core (gapless 1–59) + 32 deep-dive (gapless 60–91) + A1.5-01 retired at 900, and a live REST diff against the runtime CSV shows 0 content mismatches (a cosmetic quote-wrapping delta on all 59 core `prompt_de` rows is tracked separately — see `STATUS.md` Active item 2).
...
- [x] **R2.11 — Final independence adjudication** ✅ 2026-09-04
  ...
  **Status:** Both banks pass strict. Core (59 items): 2 flags / 1 item — A3.3-02, operand pair 4 + 5 against legacy item 50, forced by the construct (the Nachbaraufgabe to 4 + 4 can only be 4 + 5); stimulus and framing differ. Deep-dive (32 items): 0 flags. The gate was failing only because the machine-readable sidecar was missing — the adjudication itself was already written and signed in `docs/clean-room/items/A3.3-02.md` and transcribed into `math_app/Research/diagnostic_core_v1.adjudicated` on 2026-09-04.
  **⚠️ Re-key after any CSV regeneration:** the runtime CSV has no item-ID column, so the sidecar keys on row index — the 59-vs-60 delta shifted the A3.3-02 adjudication from `20` to `19`, and it was re-keyed on 2026-09-04 (both commands below re-run, exit 0). Re-key again after any future regeneration that changes row order.
