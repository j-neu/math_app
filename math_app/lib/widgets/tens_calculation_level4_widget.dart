...
class TensCalculationLevel4Widget extends StatefulWidget {
  final Function(bool) onComplete;

  const TensCalculationLevel4Widget({
    Key? key,
    required this.onComplete,
  }) : super(key: key);

  @override
  State<TensCalculationLevel4Widget> createState() => _TensCalculationLevel4WidgetState();
}

class _TensCalculationLevel4WidgetState extends State<TensCalculationLevel4Widget> {
  late int _tensA;
  late int _tensB;
  ...
  final TextEditingController _tensResultController = TextEditingController();
  ...
  void _generateProblem() {
    final random = Random();
    _tensA = 2 + random.nextInt(8); // 2-9
    _tensB = 1 + random.nextInt(_tensA); // 1 to A-1
    
    _tensAController.clear();
    _tensBController.clear();
    _tensResultController.clear();
  ...
  void _checkAnswer() {
    ...
    final inputRes = int.tryParse(_tensResultController.text);
    final inputFinal = int.tryParse(_finalResultController.text);

    if (inputA == null || inputB == null || inputRes == null || inputFinal == null) return;

    final correctRes = _tensA - _tensB;
    final correctFinal = correctRes * 10;

    if (inputA == _tensA && inputB == _tensB && inputRes == correctRes && inputFinal == correctFinal) {
      setState(() {
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    const Text(' Z  =  ', style: TextStyle(fontSize: 20)),
                    _buildSmallInput(_tensResultController),
                    const Text(' Z', style: TextStyle(fontSize: 20)),
                  ],
                ),
              ],
